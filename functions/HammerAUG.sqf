...
_LMG = ["RDS_M2StaticMG_MiniTripod_FIA","RDS_M2StaticMG_MiniTripod_CSAT","RHS_NSV_TriPod_MSV","RDS_KORD_FIA","RDS_DSHkM_Mini_TriPod_FIA","RDS_Metis_FIA","RDS_KORD_CSAT","RDS_DSHkM_Mini_TriPod_CSAT","RDS_Metis_CSAT","RDS_MK19_TriPod_FIA"];
...
_add = _veh addAction ["",{
	...
	if(( typeNAME(_veh getVariable["offAug_attached",false]) == "BOOL")  AND ((count _NO) >= 1)) then {
		...
		//Event Handler
		sleep 1;
		[[(_NO select 0),"GetOut",{(_this select 2) setPos ((_this select 0) modelToWorld [-3,-1.2,-1])},'EH_GETOUT'],"mpAddEventHand",true,true] spawn BIS_fnc_MP;
...
},[_veh,_L,_MG,_M,_LMG,_SPG,_ZU],1.5,true,true,"","((speed _target <= 1) AND (speed _target >= -1)) and (_this distance _target < 5)"];
	...
	if (speed _veh <= 1 AND speed _veh >= -1 ) then {
		if( typeNAME(_veh getVariable["offAug_attached",false]) == "OBJECT")  then{
			...
			if(isNil '_EH_OUT') then {
			 [[ _aug,'GetOut',{(_this select 2) setPos ((_this select 0) modelToWorld [-3,-1.2,-1])},'EH_GETOUT'],"mpAddEventHand",true,true] call BIS_fnc_MP;
			 };
		}else{
