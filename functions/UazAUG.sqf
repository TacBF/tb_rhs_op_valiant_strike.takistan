//V2.0
//Add this to the INIT line of the vehicle you want to use nul = [this] execVM "UazAUG.sqf";
...
_LMG = ["RDS_Metis_FIA","RDS_Metis_CSAT","RDS_AGS_CSAT"];
...
_add = _veh addAction ["",{
	...
	if(( typeNAME(_veh getVariable["offAug_attached",false]) == "BOOL")  AND ((count _NO) >= 1)) then {
		...
		sleep 1;
		//Event Handler
		[[ (_NO select 0),'GetOut',{(_this select 2) setPos ((_this select 0) modelToWorld [-3,-1.2,-1])},'EH_GETOUT'],"mpAddEventHand",true,true] call BIS_fnc_MP;
...
},[_veh,_L,_MG,_M,_LMG,_SPG],1.5,true,true,"","((speed _target <= 1) AND (speed _target >= -1)) and (_this distance _target < 5)"];
