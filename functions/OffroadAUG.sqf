...
_add = _veh addAction ["",{
	...
	if(( typeNAME(_veh getVariable["offAug_attached",false]) == "BOOL")  AND ((count _NO) >= 1)) then {
		...
		//Event Handler
		sleep 1;
		[[(_NO select 0),"GetOut",{(_this select 2) setPosATL ((_this select 0) modelToWorld [-3,-1.2,-1])},'EH_GETOUT'],"mpAddEventHand",true,true] spawn BIS_fnc_MP;
		 _veh setVariable["offAug_attached",(_NO select 0),true];
		 //(_NO select 0) setVariable ["ICE_towed", false, true];
